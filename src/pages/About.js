...
function About({ dark, toggleDarkMode }) {
  ...
  const Comment = [
    {
      desc: "‘디자인뿐만이 아니라 내가 직접 페이지를 구현하여 개발한다면 어떨까?’",
      desc2: "저는 프론트엔드 개발 분야에서 계속해서 성장하고 최신 웹 개발 기술과 트렌드를 습득합니다.",
      desc3 : "개발자 커뮤니티에 기여하고 프로젝트에 참여하여 지식을 공유하고 배울준비가 되어있습니다.  저는 그림에서 현실로의 변화를 만들어냅니다.",
      icon : faGithub,
  ...
  const data = [
    ...
    },
    {
      ...
      desc: "제가 자신있는 협업툴은요",
      desc2: ["figma", "npm(Node Package Manager)", "Yarn","Font Awesome", "Git", "GitHub", "Notion", "Vercel"],
      desc3: ["figma","figma를 사용하여 디자인 프로세스를 간편하게 디자인 할 수 있습니다. 실시간 협업과 간편한 인터페이스 덕분에, 나만의 작업실을 마련하고 창의적인 디자인을 자유롭게 펼칠 수 있습니다.","npm(Node Package Manager)", "npm을 통해 글로벌 및 로컬로 패키지를 설치하는 방법을 이해하고, 각각의 장단점을 고려하여 적절히 활용할 수 있습니다.", "Yarn","Yarn 스크립트를 통해 프로젝트 빌드, 테스트, 배포 등의 작업을 자동화하고 효율적으로 수행할 수 있습니다.","Font Awesome","Font Awesome을 사용하여 독특하고 풍부한 디자인을 만들 수 있습니다.","Git","Git을 통해 원격 저장소와 협업하고, 팀원들과 코드를 효과적으로 공유하며 변경 사항을 통합할 수 있습니다.","GitHub","Git을 기반으로 하는 코드 호스팅 플랫폼을 사용해 Git으로 관리되는 프로젝트를 온라인 상에서 저장소(repository)로 호스팅하고, 다른 개발자들과 협업할 수 있습니다.","Notion","Notion을 사용하여 다양한 형식의 문서를 작성하고 노트테이킹을 수행합니다. 텍스트, 이미지, 체크리스트 등을 통합하여 효과적으로 정보를 기록할 수 있습니다.","Vercel","Vercel을 사용하여 정적인 웹사이트 및 서버리스 함수를 효과적으로 배포하고 관리할 수 있습니다."]
    },
    {
      title : "EDUCTION",
    ...
    <div className={`w-full h-full mx-auto pt-[120px] md:pt-[180px] lg:pt-[200px] ${dark ? `bg-[#010b1a]` : `bg-white`}`}>
      ...
      <div className='flex mx-auto md:mb-20 max-w-7xl'>
        <div className="flex flex-wrap justify-between mx-auto text-center">
          <div className='mx-auto pb-5 mb-10 text-justify border-b basis-[78%] md:pb-7 lg:pb-10 md:basis-[90%] lg:basis-full'>
            <p className={`mb-5 ${dark ? `text-[#bdbdbd]` : `text-black`} text-xl text-center md:text-3xl lg:text-4xl md:mb-20`}>디자이너에서 <span className={`font-bold ${dark ? `text-[#ddd]`: ``} text-[#242424]`}>개발자 </span>로의 성장</p>
            {
              Comment.map((e,i)=>{
                return(
                  <div key={i} className=''>
                    <p className={`text-xs mb-5 lg:mb-9 md:text-xl md:w-[95%] md:mx-auto lg:w-full lg:text-[28px] ${dark ? `text-[#e6e6e6]` : ``}`}>{e.desc}</p>
                    <p className={`text-xs mb-5 lg:mb-9 md:text-xl md:w-[95%] md:mx-auto lg:w-full lg:text-[28px] ${dark ? `text-[#e6e6e6]` : ``}`}>{e.desc2}</p>
                    <p className={`text-xs mb-5 lg:mb-9 md:text-xl md:w-[95%] md:mx-auto lg:w-full lg:text-[28px] lg:leading-10 leading-1 ${dark ? `text-[#e6e6e6]` : ``}`}>{e.desc3}</p>
                  </div>
                )
              })
            }
              ...
              <div className='pb-2 md:pb-4 lg:pb-6'>
                {
                  ...
                  Array(e.desc3.length).fill().map((_,index)=>{
                    return(
                      <ul key={index} className={`text-xs md:text-base lg:odd:text-[22px] odd:font-bold lg:text-xl mt-2 md:mt-3 lg:mt-5 mx-auto w-[75%] md:w-[80%] lg:w-full even:border-b ${dark ? `text-[#ddd]` : ``}`}>
